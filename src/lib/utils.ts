...
export const formatDate = (date: string | Date, includeRelative = false) => {
  ...
  if (includeRelative) {
    ...
    const fullDate = targetDate.toLocaleDateString(DATE_CONFIG.dateLocale, { ...DATE_CONFIG.dateFormat, timeZone: 'UTC' });

    return `${fullDate} (${relativeDate})`;
  }

  return targetDate.toLocaleDateString(DATE_CONFIG.dateLocale, { ...DATE_CONFIG.dateFormat, timeZone: 'UTC' });
};
