...
// Shared journal data used by the Kit integration
export type Post = {
  metadata: {
    title: string;
    publishedAt: string;
...
export function getKitToken(): string {
  const token = process.env.KIT_API_KEY || import.meta.env?.KIT_API_KEY;
  if (!token) {
...
async function kitFetch<T>(path: string, init?: RequestInit): Promise<T> {
  ...
  if (!response.ok) {
    const errorMsg = (data as { errors?: string[] })?.errors?.[0] || response.statusText;
...
/**
 * Lookups must succeed before syncing: an upstream failure is not an empty list.
 */
export async function getSyncedPosts(posts: Post[]): Promise<KitSyncStatus[]> {
  const broadcasts = await getKitBroadcasts();
  return posts.flatMap((post) => {
    const broadcast = broadcasts.find((candidate) => candidate.subject.trim() === post.metadata.title.trim());
    return broadcast ? [{
      slug: post.slug,
      kitId: broadcast.id,
      syncedAt: broadcast.created_at,
    }] : [];
  });
}
